...
export function ProjectGrid() {
  ...
  const [filter, setFilter] = useState<"all" | "featured" | "ai" | "dbms" | "personal">("all");
      ...
      >
        <div className="flex justify-center px-4">
          <div className="flex gap-3 flex-wrap justify-center backdrop-blur-sm saturate-200 p-3 rounded-2xl md:rounded-full shadow-[inset_0_1px_0_0_rgba(255,255,255,0.2),inset_0_0_0_1px_rgba(255,255,255,0.1)] shadow-lg">
            {(["all", "featured", "ai", "dbms", "personal"] as const).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={cn(
                  "px-4 md:px-6 py-2 rounded-full transition-all duration-300 text-xs md:text-sm capitalize",
                  filter === f
                    ? "bg-white text-black font-medium shadow-lg"
                    : "bg-neutral-800 text-white hover:bg-neutral-700"
                )}
              >
                {f === "all" ? "All Projects" : f === "ai" ? "AI Integrated" : f === "dbms" ? "Dbms Dashboard" : f === "personal" ? "Personal Project" : f}
              </button>
            ))}
          </div>
        </div>
